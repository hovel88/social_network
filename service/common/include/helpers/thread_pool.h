...
using OnLoggingFunc  = std::function<void(const std::string&)>;

class ThreadPool
{
protected:
    struct TaskMeta {
        TaskMeta(uint64_t id, OnCompleteFunc& cb, OnLoggingFunc& log)
        :   id_(id), cb_(cb), log_(log) {}
...
public:
    ...
    template<typename Func, typename... Args>
    std::optional<uint64_t> add_task(OnCompleteFunc cb, OnLoggingFunc log, Func func, Args&&... args)
    {
        uint64_t id = 0;
        bool refused = false;

        {
            ...
            if (tasks_.size() < tasks_max_capacity_) {
                auto& t = tasks_.emplace(PooledTask(func, std::forward<Args>(args)...), TaskMeta(id, cb, log));
