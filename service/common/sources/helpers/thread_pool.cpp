...
void ThreadPool::run(const std::string thread_name)
{
    ...
    while (!threads_stop_) {
        ...
        if (!tasks_.empty() && !threads_stop_) {
            ...
            if (task.second.log_) {
                task.second.log_(std::format("thread {}, start processing task #{}", thread_name, task.second.id_));
            }
            try {
                task.first();
            }
                ...
                if (task.second.log_) {
                    task.second.log_(std::format("thread {} while processing task #{}, exception: {}", thread_name, task.second.id_, ex.what()));
                }
            }
            if (task.second.log_) {
                task.second.log_(std::format("thread {}, end processing task #{}", thread_name, task.second.id_));
            }
