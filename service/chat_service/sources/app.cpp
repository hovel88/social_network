...
:   logger_(Configuration::instance().get_logger()),
    http_server_(std::shared_ptr<drogon::HttpAppFramework>(&drogon::app(), [](drogon::HttpAppFramework*){})) // deleter (пустой), т.к. за жизненный цикл объекта отвечает синглтон, а не shared_ptr
{}
...
void App::run()
{
    ...
    try {
        ...
        // создаем сервисы
        service_inmem_       = std::make_shared<InMemService>(logger_);
        service_auth_        = std::make_shared<AuthService>(logger_, metrics_, service_inmem_);
        service_dialog_http_ = std::make_unique<HttpDialogService>(logger_, metrics_, service_inmem_);
        service_dialog_grpc_ = std::make_unique<GrpcDialogService>(logger_, service_inmem_);
...
void App::grpc_start()
{
    ...
    try {
        grpc_server_thread_ = std::thread([this]()->void {
            ...
            builder.RegisterService(service_dialog_grpc_.get());
