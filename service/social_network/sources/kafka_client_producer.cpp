...
KafkaProducer::KafkaDeliveryReporter::KafkaDeliveryReporter()
:   logger_(Configuration::instance().get_logger())
{
}

void KafkaProducer::KafkaDeliveryReporter::dr_cb(RdKafka::Message& message)
{
...
void KafkaProducer::initialize(const std::string& brokers)
{
    std::string errstr;
...
void KafkaProducer::produce_to_user_topics(const std::vector<std::string>& friend_ids,
                                           const std::string& message)
{
    for (const auto& friend_id : friend_ids) {
        std::string topic = "user_" + friend_id + "_posts";
        produce(topic, message);
    }

    // можно также отправлять сообщения в общий топик, например, для мониторинга
    // produce("all_posts", message);
}
